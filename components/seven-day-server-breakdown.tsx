...
export function SevenDayServerBreakdown() {
  ...
  async function handleToggle(server: ServerOption) {
    ...
    try {
      const response = await fetch(`/api/revenue/seven-day/${server}`, {
        method: "GET",
  ...
  return (
    <section className="mt-6 rounded-[1.9rem] border border-stone-200 bg-stone-50/80 p-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.7),0_18px_45px_-36px_rgba(68,46,20,0.45)] sm:p-6">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h3 className="text-lg font-semibold text-stone-950">7-Day Total</h3>
          <p className="mt-1 text-sm text-stone-600">
            Open only the servers you want to inspect. Each server fetches its
            own 7-day breakdown when requested.
          </p>
        </div>
            ...
            >
              ...
              {serverState.expanded ? (
                <div className="mt-3 space-y-2">
                  ...
                  {serverState.rows?.map((row, index) => (
                    <div
