...
import { deleteRevenueEntry, updateRevenueEntry } from "@/app/actions";
...
export function RevenueEntryRow({ entry }: RevenueEntryRowProps) {
          ...
          <form action={updateAction} className="space-y-4">
            ...
            {updateState.success ? (
              <p className="rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-700">
                {updateState.message} You can close this form or keep adjusting the entry.
              </p>
            ) : null}
          </form>
        </td>
  ...
  return (
    <tr className="align-top shadow-sm">
      <td className="rounded-l-3xl border-y border-l border-stone-200 bg-stone-50 px-4 py-4 font-semibold text-stone-950">
        {entry.server}
      </td>
